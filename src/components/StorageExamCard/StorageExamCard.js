...
import "./StorageExamCard.css";
import constants from "../../utils/constants";
import Noti from "../../utils/Noti";
import axios from "axios";
import { useNavigate } from "react-router-dom";

const StorageExamCard = ({ item, storage, setReload }) => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const res = await axios.get(
                    `${API}/examResult/get-one?examId=${item._id}`
                );
                let data = res.data.data;
                setExamResult(data);
            } catch (err) {
    ...
    return (
        <div
            className={`storage-exam-card ${item.level}`}
...
export default StorageExamCard;
